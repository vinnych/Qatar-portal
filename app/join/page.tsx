...
export default function JoinPage() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen pt-20 pb-20 px-4 relative">
      {/* Header */}
      <div className="mb-16 animate-in fade-in slide-in-from-top-4 duration-1000 text-center">
        <h1 className="text-xs tracking-[0.6em] uppercase font-bold text-accent mb-4">Community</h1>
        <p className="text-3xl font-black serif text-foreground">Boutique Enquiry</p>
      </div>
      ...
      <div className="w-full max-w-xl glass p-10 sm:p-16 rounded-[3rem] border-brand-gold/20 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-200 shadow-2xl">
        {inviteStatus === "sent" ? (
          <div className="text-center py-10">
            ...
            <h2 className="text-2xl font-black serif text-foreground mb-4">Inquiry Received</h2>
          ...
          <form className="space-y-6" onSubmit={handleInvite}>
            <div>
              <label className="block text-[10px] font-black uppercase tracking-[0.3em] text-accent mb-3 ml-4">Full Name</label>
              <input
                ref={nameRef}
                type="text"
                placeholder="Your Name..."
                className="w-full bg-white/70 dark:bg-brand-obsidian/40 border border-brand-gold/20 rounded-full px-8 py-4 text-sm focus:outline-none focus:border-brand-gold transition-all shadow-inner"
              />
            </div>
            <div>
              <label className="block text-[10px] font-black uppercase tracking-[0.3em] text-accent mb-3 ml-4">Email Address</label>
              <input
                ref={emailRef}
                type="email"
            ...
            >
              {inviteStatus === "sending" ? "Processing..." : "Submit Inquiry"}
            </button>
            {inviteStatus === "error" && (
              <p className="text-center text-xs text-red-500 font-bold tracking-widest uppercase mt-4">Something went wrong. Please try again.</p>
            )}
          </form>
        )}
      </div>
